...
class AuthController extends GetxController implements GetxService {
  ...
  Future<ResponseModel> login({
    ...
    // <-- IMPORTANT : si succès, on envoie le token push en "safe"
    if (responseModel.isSuccess) {
      _sendPushTokenSafely(); // pas de await pour ne JAMAIS bloquer la navigation
    }
  ...
  Future<void> _sendPushTokenSafely() async {
    try {
      // iOS : APNS token (souvent null si l’utilisateur refuse, ou sur simulateur)
      String? apnsToken;
      if (Platform.isIOS) {
        try {
      ...
      // FCM token (Android + iOS si autorisé)
      final String? fcmToken = await FirebaseMessaging.instance.getToken();

      // Aucun token → on NE jette pas d’erreur, on ignore simplement
      if (fcmToken == null && apnsToken == null) {
        return;
      }

      // Si ton service sait déjà gérer la récupération du token, laisse comme ça :
      await authServiceInterface.updateToken();

      // Sinon, si ton service a besoin du token en paramètre, utilise :
      // await authServiceInterface.updateTokenWithValue(fcmToken ?? apnsToken!);

    } catch (e, st) {
      // **Très important :** ne JAMAIS rethrow ici
      debugPrint('_sendPushTokenSafely error: $e\n$st');
    }
  }
